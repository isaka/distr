...
Distr is designed to help distribute "self-managed" ready software to [customer-controlled environments](https://glasskube.dev/blog/5-ways-to-succeed-without-access-to-customer-environments/). The key phrase here is ["self-managed ready"](https://glasskube.dev/white-paper/building-blocks/).

### What does "self-managed ready" mean?

#### Self-managed ready checklist:
...
If you're a software company who has developed a SaaS product and later been approached by a customer requesting to use your product in a "self-managed" deployment, you've likely experienced the challenges of lacking self-managed readiness. Deploying to self-managed environments or complex customer-controlled environments requires deliberate decisions and architectural considerations made early in the development process to ensure such deployments are practical.

Software built with self-managed readiness in mind from the start often reflects thoughtful design, focusing on maintainability and configurations suitable for future self-managed use cases. However, many software solutions aren't initially created with these scenarios in mind. As customer requirements evolve, SaaS products may need adjustments to support self-managed deployments. Ideally, your application should already be containerized, with a Docker Compose file or Helm chart prepared. If not, feel free to reach out for white-labeled onboarding assistance.

#### What changes are needed for self-managed readiness?

To bridge this gap, we've compiled a document outlining the foundational building blocks for achieving self-managed readiness. You can access this comprehensive [whitepaper here](https://glasskube.dev/white-paper/building-blocks/).
...
-- **"An employee of the customer"**: Select this when the end-user's team is responsible for managing deployments, and the Distr user does not have access to the deployment target. More suited towards the self-managed use case.
